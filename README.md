...
リリース版は将来的に[GitHub Releases](https://github.com/mshibagaki/HPLC_Analyzer/releases)とSemVer tagで管理する方針です（配布チャネルの決定は[`DISTRIBUTION.md`](DISTRIBUTION.md)を参照）。ただし現時点ではtagもGitHub Releaseも未作成で、この運用はまだ始まっていません。ソースディレクトリ名にはバージョン番号を含めません。現在の開発版のバージョン番号は「[Versioning](#versioning)」を参照してください。
...
- 表で選択した複数クロマトグラムを、保持時間×系列×強度の3D折れ線として表示し、PNG / SVG / PDFへ出力。表の順序・ラベル・トレース色・現在のX範囲・既存の軸タイトル／フォント／出力品質を引き継ぎ、系列軸タイトル、Z範囲、視点、アスペクト比、X/Z目盛間隔、配色、軸線幅だけをダイアログで調整（時間シフトは適用し、2D用の縦オフセットは適用しない。設定はセッション限り）
...
- 左表の「すべて表示」「すべて非表示」
- 左表の「単独」列で1件だけを一時的に表示（各行の表示チェックは変更せず、セッション限りの状態）
...
- 全体図（オーバービュー）は詳細図から独立した表示範囲を持ち、専用のホイール／スクロールバー／拡大縮小ボタンでその範囲だけを操作。境界線をドラッグして全体図と詳細図の比率を調整（表示中は比率を保持）
- 全体図の拡大縮小ボタンはY方向をゼロ（0基準線）を起点に拡大・縮小し、ベースライン付近の小さいピークが視界から外れにくい
- Y軸1を上段、Y軸2を下段へ分けた共有X軸の2パネル表示へ切替
- グラフ部分と操作・ピーク表部分の境界をドラッグして表示高さを変更
- 操作・ピーク表部分の内側でも、操作パネルとピーク表（積分リスト）の境界を別途ドラッグして高さ配分を調整
...
- 検出器の飽和でピーク頭部が平らになった場合の「飽和ピーク補正」。積分範囲を初期値とした範囲を確認・調整して実行し、飽和区間だけを非飽和部分から推定したモデル値で埋め戻す。補正後は積分範囲内の実測値＋モデル値を合わせた面積が親ピークに代わって%Areaと定量値に使われ、表・レポート・CSVで常に推定値として明示される。親ピーク自体の生値は変更されず、取り消せば元の%Areaに戻る
- 自動検出結果を編集可能な候補として追加し、手動/自動を記録
- 積分リストの複数選択、全選択、一括削除。選択後は`Delete`キーでも削除可能
- 積分リストの備考欄へ、ピーク同定結果などを直接入力
- 積分リストを別ウィンドウとして切り離して表示でき、選択・備考・範囲修正・フィット・削除が主表と常に同期
- 積分、分割、範囲修正、削除、自動検出、時間シフト、飽和ピーク補正等のUndo/Redo
...
検出実行時はキャンセル可能なダイアログが開き、データ全体または事前に選択した時間範囲、および低・中・高の感度プリセットから選べます。各感度の8つのしきい値は`設定 → 環境設定`で個別に編集・既定値へリセットでき、選んだ感度はプロジェクトの測定条件へコピーされます。
