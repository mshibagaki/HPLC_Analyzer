...
Updated: 2026-08-29 (Issue #162 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | The session-only Qt 6 preview supports shared navigation, single/overview/split views, independent Y axes, scene refresh, snapshots and the existing scientific editing workflows. Issue #160 supplies native presentation parity. Issue #162 stops constructing hidden Matplotlib traces, gradients, overlays, markers, annotations, overview traces and legends during preview refresh; only an axes/view-state skeleton remains. PNG/SVG/PDF export temporarily rebuilds the complete high-quality Matplotlib figure and then returns to the skeleton, while preview disablement or failure rebuilds the complete fallback screen. Win7 and defaults stay unchanged. The integrated 8-trace × 100,000-point offscreen benchmark proved zero hidden Matplotlib data lines and unchanged raw arrays, but native full-scene recreation was slower (1.989 s median versus 0.301 s), so this is not yet a completed high-performance replacement | Reuse/update native scene items and add view-aware data reduction so high-density refresh is faster in the integrated application. Keep the experiment opt-in; require native desktop compatibility/performance evidence before any default change |
