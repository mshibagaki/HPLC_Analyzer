...
Updated: 2026-08-29 (Issue #164 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | The session-only Qt 6 preview supports shared navigation, all display layouts, existing scientific editing workflows and native presentation parity. Issue #162 retains only a Matplotlib axes/view-state skeleton during preview, with complete high-quality artists rebuilt for figure export and fallback. Issue #164 adds peak-preserving screen-only min/max reduction and reuses detail/overview PyQtGraph trace items when trace IDs and axis ownership are compatible; topology or overlay changes safely use full reconstruction. The integrated 8-trace × 100,000-point offscreen benchmark reduced 800,000 source points to 26,256 rendered points, retained item identity and raw SHA-256, and measured 0.147 s native versus 0.299 s legacy (2.04x). Win7 and defaults stay unchanged | Extend safe item reuse to overlay-rich real projects where beneficial, then collect native desktop DPI/font/pointer/clipboard/print and representative real-data performance evidence before deciding whether to change the Windows 11 default |
