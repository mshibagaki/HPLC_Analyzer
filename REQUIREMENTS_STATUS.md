...
Updated: 2026-08-28 (Issue #134 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | Issue #134 connects a session-only experimental PyQtGraph view in MainWindow on Qt 6, with shared zoom/pan/history/reset, single/overview views with independent Y2 overview scaling, refreshed scenes, and current-view snapshots. Editing tools, rectangle zoom, plot configuration, split Y1/Y2 view, unavailable dependencies, and renderer failures return to Matplotlib with an explanation. Win7 and defaults stay unchanged. Hidden Matplotlib objects are still built for compatibility/export; this is not a completed high-performance replacement | Implement native editing and split-view parity, exact presentation parity (legend placement/fonts/ticks), then eliminate hidden Matplotlib screen construction and benchmark the integrated application. Keep the experiment opt-in and do not change the default until physical compatibility evidence exists |
