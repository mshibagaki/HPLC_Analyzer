...
Updated: 2026-08-29 (Issue #152 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | The session-only Qt 6 preview supports shared navigation, single/overview/split views, independent Y axes, scene refresh and snapshots. Native vertical-marker editing, fraction ranges, manual integration, existing-peak range editing, peak splitting and Issue #152 trace movement share existing Undo/Redo and project persistence. Integration/range edits use existing calculation/baseline/time-shift callbacks and only the selected visible trace on its own split panel; changed targets cancel pending spans, including changed peak identity/content. Range editing preserves the peak ID/settings and rolls back invalid ranges. Peak splitting dispatches the existing nearest-valid-sample calculation, preserves the parent baseline/notes/source and is atomic on recalculation failure. Trace movement gives native X/Y/both feedback for detail and overview, commits through the existing shift/offset/peak recalculation callback, and restores uncommitted changes on target/mode/view/focus/Escape/backend changes or failure. Shared drag feedback/cancellation preserves fraction semantics. Split-view B% remains both-panels-or-neither. Text editing, rectangle zoom, plot configuration and failures still return to Matplotlib. Win7 and defaults stay unchanged. Hidden Matplotlib objects remain for compatibility/export; this is not a completed high-performance replacement | Implement the remaining native editing and exact presentation parity (legend placement/fonts/ticks), then eliminate hidden Matplotlib screen construction and benchmark the integrated application. Keep the experiment opt-in and do not change the default until physical compatibility evidence exists |
