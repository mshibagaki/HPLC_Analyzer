...
Updated: 2026-08-28 (Issue #132 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | Production-used scene covers all static content. The optional PyQtGraph consumer applies independent ranges/overview, renders overlays/snapshots, and dispatches shared pointer events. Issue #132 adds an opt-in standalone navigation controller for cursor-targeted wheel zoom, left-button axis pan, overview centering, and home/back/forward. Explicit event ownership prevents duplicate native pan/zoom; observers remain non-consuming without a controller. Qt Y coordinates are normalized and B% ranges survive pan/history. MainWindow toolbar/editing-tool routing and backend selection/fallback remain unconnected | Connect the controller to MainWindow actions and remaining editing/split-view behavior, then expose the renderer behind an opt-in Windows 11 setting with automatic Matplotlib fallback; do not change the default until physical compatibility evidence exists |
