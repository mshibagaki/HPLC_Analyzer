...
Updated: 2026-08-29 (Issue #154 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | The session-only Qt 6 preview supports shared navigation, single/overview/split views, independent Y axes, scene refresh and snapshots. Native vertical markers, fraction ranges, integration editing/splitting, trace movement and Issue #154 text-label creation/hit-testing/movement/edit/delete share existing Undo/Redo and project persistence. Scientific edits use existing calculation/baseline/time-shift callbacks and only the selected visible trace on its own split panel. Changed targets cancel pending spans; invalid range/split/move operations roll back atomically. Trace movement gives native X/Y/both feedback for detail and overview. Text labels retain fixed screen size and Y1/Y2 ownership, reuse the existing dialog, and restore uncommitted movement on focus/Escape/view/backend changes or failure. Split-view B% remains both-panels-or-neither. Rectangle zoom, plot configuration and failures still return to Matplotlib. Win7 and defaults stay unchanged. Hidden Matplotlib objects remain for compatibility/export; this is not a completed high-performance replacement | Implement rectangle zoom/configuration and exact presentation parity (legend placement/fonts/ticks), then eliminate hidden Matplotlib screen construction and benchmark the integrated application. Keep the experiment opt-in and do not change the default until physical compatibility evidence exists |
