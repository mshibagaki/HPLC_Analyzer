...
- The non-GUI suite passed 123 tests for the Issue #122 candidate, including normalized backend-neutral overview full/detail-window state, bounded home/back/forward history, axis-pan state/calculations, immutable pointer coordinates and hit targets, optional-renderer isolation/pinning, backend-neutral scene contracts, update metadata, project migrations, numerical invariance, import, presets, and release-source contracts.
- The Windows/PySide6 offscreen GUI suite passed 118 tests for the Issue #122 candidate, including overview/detail patch updates and click navigation, toolbar history, raw and normalized pan paths, marker/annotation targets, normalized pointer paths, the complete static PyQtGraph scene consumer, and a non-empty 800×500 Qt snapshot.
...
- PyQtGraph MainWindow adapter behind an opt-in Windows 11 setting with Matplotlib fallback. The optional consumer renders the complete static production scene with independent Y1/Y2/B% and snapshot; pointer handling, target routing, pan, history, and overview state are backend-neutral. MainWindow backend selection and fallback still need connection. Changing defaults still requires compatibility evidence.
