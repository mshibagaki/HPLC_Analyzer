...
Updated: 2026-08-28 (Issue #116 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | Production-used scene covers all static content and has an optional PyQtGraph consumer with independent Y1/Y2/B%, full overlays, and snapshot. Wheel/press/motion/release input and marker/annotation hit targets use immutable backend-neutral contracts; each renderer resolves its own hit targets at the adapter boundary. Toolbar navigation, overview-window rendering updates, and MainWindow backend switching remain backend-specific | Extract the navigation and overview-update contracts next, then connect the consumer behind an opt-in Windows 11 setting with automatic Matplotlib fallback; do not change the default until physical compatibility evidence exists |
