...
Updated: 2026-08-29 (Issue #156 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | The session-only Qt 6 preview supports shared navigation, single/overview/split views, independent Y axes, scene refresh and snapshots. Native vertical markers, fraction ranges, integration editing/splitting, trace movement, text-label editing and Issue #156 rectangle zoom share existing display history, Undo/Redo and project persistence as applicable. Rectangle zoom supports X, Y, X+Y and automatic modes; split-view Y zoom changes only the dragged panel, while Home/Back/Forward and cancellation remain backend-neutral. Scientific edits use existing calculation/baseline/time-shift callbacks and only the selected visible trace on its own split panel. Invalid or changed-target edits roll back atomically. Text labels retain fixed screen size and Y1/Y2 ownership. Split-view B% remains both-panels-or-neither. Plot configuration and failures still return to Matplotlib. Win7 and defaults stay unchanged. Hidden Matplotlib objects remain for compatibility/export; this is not a completed high-performance replacement | Implement native plot configuration and exact presentation parity (legend placement/fonts/ticks), then eliminate hidden Matplotlib screen construction and benchmark the integrated application. Keep the experiment opt-in and do not change the default until physical compatibility evidence exists |
