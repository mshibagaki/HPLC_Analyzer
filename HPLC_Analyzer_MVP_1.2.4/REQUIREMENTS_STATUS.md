...
Updated: 2026-08-28 (Issue #118 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | Production-used scene covers all static content and has an optional PyQtGraph consumer with independent Y1/Y2/B%, full overlays, and snapshot. Pointer input, marker/annotation hit targets, and axis-target pan state/calculations use backend-neutral contracts. Matplotlib toolbar home/back/forward history, overview-window rendering updates, and MainWindow backend switching remain backend-specific | Extract the remaining history and overview-update contracts next, then connect the consumer behind an opt-in Windows 11 setting with automatic Matplotlib fallback; do not change the default until physical compatibility evidence exists |
