...
| High | Replace the interactive Matplotlib screen renderer | The session-only Qt 6 preview supports shared navigation, single/overview/split views, independent Y axes, scene refresh and snapshots. Native vertical-marker editing and Issue #142 fraction-range drag selection share existing Undo/Redo and project persistence. Fraction drags have transient feedback and cancel on Escape, focus/viewport/scene/view/mode changes; existing project-wide range semantics and interval calculations are unchanged. Split-view B% remains both-panels-or-neither. Other editing tools, rectangle zoom, plot configuration and failures still return to Matplotlib. Win7 and defaults stay unchanged. Hidden Matplotlib objects remain for compatibility/export; this is not a completed high-performance replacement | Implement the remaining native editing and exact presentation parity (legend placement/fonts/ticks), then eliminate hidden Matplotlib screen construction and benchmark the integrated application. Keep the experiment opt-in and do not change the default until physical compatibility evidence exists |
