...
- The non-GUI suite passed 124 tests for the Issue #136 candidate from the repository root, including B% range preservation during shared axis pan, the root-layout and workflow contracts, normalized backend-neutral overview full/detail-window state, bounded home/back/forward history, axis-pan state/calculations, immutable pointer coordinates and hit targets, optional-renderer isolation/pinning, backend-neutral scene contracts, update metadata, project migrations, numerical invariance, import, presets, and release-source contracts.
- The Windows/PySide6 offscreen GUI suite passed 124 tests for Issue #136. New native-event coverage exercises vertical-marker hover/place/select/Delete, selected Y-axis ownership, selection tolerance after zoom, selection without dirty/undo changes, Undo/Redo, project save/reload, raw-array invariance, and key/cursor failure cleanup. The prior MainWindow navigation, snapshot/export, scene refresh, fallback and default-Matplotlib regressions also pass. This is not physical mouse/device validation.
- Issue #136 passed syntax compilation and `scripts/ci_validate.py --offline-assets required`, including the locally supplied Windows 7 manifest/wheelhouse. Earlier Issue #134 offscreen MainWindow PNGs were visually inspected with local Meiryo explicitly registered and axis/tick spacing corrected. Native desktop fonts/DPI, lower-control sizing and physical printing remain open as recorded above. No installer or physical OS/device validation was performed.
...
- The experimental PyQtGraph MainWindow view has shared navigation/snapshots and, since Issue #136, native vertical-marker editing through the existing undo-aware Project methods. Other unsupported editing/split-view/plot configuration and renderer failures still return to Matplotlib. Next source work is the remaining editing and split-view parity, exact legend/font/tick presentation, removal of hidden Matplotlib screen construction, and integrated performance measurement. Default changes still require physical compatibility evidence.
