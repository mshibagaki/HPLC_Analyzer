...
Updated: 2026-08-28 (Issue #138 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | The session-only Qt 6 preview supports shared navigation, single/overview views, independent Y2 overview, scene refresh and snapshots. Native vertical-marker editing shares Undo/Redo and project persistence. Issue #138 adds aligned split Y1/Y2 panels, shared X with pane-specific Y zoom/pan, B% on the selected trace's pane, pane-scoped marker placement/hit testing, and layout switching without leaving the preview. Other editing tools, rectangle zoom, plot configuration and failures still return to Matplotlib. Win7 and defaults stay unchanged. Hidden Matplotlib objects remain for compatibility/export; this is not a completed high-performance replacement | Implement the remaining native editing and exact presentation parity (legend placement/fonts/ticks), then eliminate hidden Matplotlib screen construction and benchmark the integrated application. Keep the experiment opt-in and do not change the default until physical compatibility evidence exists |
