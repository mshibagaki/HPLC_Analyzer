...
4. `詳細・定量条件`でサンプル、測定条件、試料情報を設定します。複数データは`条件の一括入力・プリセット`の表から直接編集でき、Shift/Ctrlでの複数選択とCtrl+C/Ctrl+Vに対応します。条件／グラジエントプリセットは`内容・差分…`から適用前に確認できます。入力エラーがあれば表やProjectへ一部適用せず、該当セルを表示します。
