...
- The non-GUI suite passed 124 tests for the Issue #130 candidate from the repository root, including the root-layout and workflow contracts, normalized backend-neutral overview full/detail-window state, bounded home/back/forward history, axis-pan state/calculations, immutable pointer coordinates and hit targets, optional-renderer isolation/pinning, backend-neutral scene contracts, update metadata, project migrations, numerical invariance, import, presets, and release-source contracts.
- The Windows/PySide6 offscreen GUI suite passed 119 tests for the Issue #130 candidate from the repository root. Coverage includes real Qt viewport press/double-click/move/release/wheel delivery, callback disconnection/close lifecycle, mouse-button and modifier normalization, plus the existing navigation, independent X/Y1/Y2/B% ranges, overview detail window, and non-empty 800×500 Qt snapshot. This remains offscreen evidence, not physical mouse/device validation.
...
- PyQtGraph MainWindow adapter behind an opt-in Windows 11 setting with Matplotlib fallback. The optional consumer renders the static scene, applies independent ranges and overview state, and dispatches viewport events through shared callbacks. Application navigation-action routing is still unconnected and must explicitly avoid duplicate native pan/zoom handling. MainWindow backend selection and fallback also remain unconnected. Changing defaults still requires compatibility evidence.
