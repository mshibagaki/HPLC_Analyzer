...
- The non-GUI suite passed 124 tests for the Issue #132 candidate from the repository root, including B% range preservation during shared axis pan, the root-layout and workflow contracts, normalized backend-neutral overview full/detail-window state, bounded home/back/forward history, axis-pan state/calculations, immutable pointer coordinates and hit targets, optional-renderer isolation/pinning, backend-neutral scene contracts, update metadata, project migrations, numerical invariance, import, presets, and release-source contracts.
- The Windows/PySide6 offscreen GUI suite passed 120 tests for the Issue #132 candidate from the repository root. Coverage includes real Qt-event navigation ownership, single-step independent Y2 zoom, Y-up pan direction, B% range/history preservation, overview centering, home/back/forward, ignored wheel during drag, and retained forward history on bounded no-op zoom. Existing callback lifecycle, normalization, static rendering, and 800×500 Qt snapshot coverage also passes. This remains offscreen evidence, not physical mouse/device validation.
- Issue #132 also passed syntax compilation and `scripts/ci_validate.py --offline-assets required`, including the locally supplied Windows 7 manifest/wheelhouse. No installer or physical OS/device validation was performed.
...
- PyQtGraph MainWindow adapter behind an opt-in Windows 11 setting with Matplotlib fallback. The optional consumer renders the static scene and supports independent ranges/overview and shared viewport events. Issue #132 adds standalone navigation with explicit event ownership, cursor-targeted zoom, axis pan, overview centering, and home/back/forward. MainWindow toolbar/editing-tool routing, remaining split-view behavior, backend selection, and fallback remain unconnected. Changing defaults still requires compatibility evidence.
