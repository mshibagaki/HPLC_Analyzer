...
Updated: 2026-08-28 (Issue #136 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | The session-only Qt 6 preview supports shared navigation, single/overview views, independent Y2 overview, scene refresh and snapshots. Issue #136 adds native vertical-marker hover/place/select/Delete with pixel-based hit testing, shared Undo/Redo and existing project persistence. Other editing tools, rectangle zoom, plot configuration, split Y1/Y2 and failures still return to Matplotlib. Win7 and defaults stay unchanged. Hidden Matplotlib objects remain for compatibility/export; this is not a completed high-performance replacement | Implement the remaining native editing and split-view parity, exact presentation parity (legend placement/fonts/ticks), then eliminate hidden Matplotlib screen construction and benchmark the integrated application. Keep the experiment opt-in and do not change the default until physical compatibility evidence exists |
