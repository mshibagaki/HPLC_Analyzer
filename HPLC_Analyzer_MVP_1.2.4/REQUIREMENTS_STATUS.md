...
Updated: 2026-08-28 (Issue #120 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | Production-used scene covers all static content and has an optional PyQtGraph consumer with independent Y1/Y2/B%, full overlays, and snapshot. Pointer input, hit targets, axis-target pan, and bounded home/back/forward view history use backend-neutral contracts. Overview-window rendering updates and MainWindow backend switching remain backend-specific | Extract the overview-update contract next, then connect the consumer behind an opt-in Windows 11 setting with automatic Matplotlib fallback; do not change the default until physical compatibility evidence exists |
