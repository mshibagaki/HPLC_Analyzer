...
Updated: 2026-08-29 (Issue #160 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | The session-only Qt 6 preview supports shared navigation, single/overview/split views, independent Y axes, scene refresh and snapshots. Native vertical markers, fraction ranges, integration editing/splitting, trace movement, text-label editing and rectangle zoom share existing display history, Undo/Redo and project persistence as applicable. Both toolbar configuration entries reuse the persisted axes/label-style dialog without leaving the preview. Issue #160 applies persisted axis-title and tick-label typography/colors, manual/automatic X tick spacing and all six legend positions/typography; outside-right uses an unclipped layout column. Scientific edits use existing callbacks and invalid or changed-target edits roll back atomically. Split-view B% remains both-panels-or-neither. Renderer failures still return to Matplotlib. Win7 and defaults stay unchanged. Hidden Matplotlib objects remain for compatibility/export; this is not a completed high-performance replacement | Eliminate hidden Matplotlib screen construction and benchmark the integrated application, while retaining Matplotlib only for compatibility/export paths that still require it. Keep the experiment opt-in and do not change the default until physical compatibility evidence exists |
