...
- The non-GUI suite passed 122 tests for the Issue #120 candidate, including bounded backend-neutral home/back/forward history, axis-pan state/calculations, immutable pointer coordinates and hit targets, optional-renderer isolation/pinning, backend-neutral scene contracts, update metadata, project migrations, numerical invariance, import, presets, and release-source contracts.
- The Windows/PySide6 offscreen GUI suite passed 118 tests for the Issue #120 candidate, including toolbar home/back/forward over wheel zoom, raw Matplotlib pan and normalized pan events, marker/annotation targets, normalized pointer paths, the complete static PyQtGraph scene consumer, and a non-empty 800×500 Qt snapshot.
...
- PyQtGraph MainWindow adapter behind an opt-in Windows 11 setting with Matplotlib fallback. The optional consumer renders the complete static production scene with independent Y1/Y2/B% and snapshot; pointer handling, target routing, pan, and home/back/forward history accept backend-neutral input. Overview rendering updates and backend switching still need extraction/adaptation. Changing defaults still requires compatibility evidence.
