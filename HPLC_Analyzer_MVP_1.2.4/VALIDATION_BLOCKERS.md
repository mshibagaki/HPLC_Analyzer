...
- The non-GUI suite passed 116 tests for the Issue #104 candidate, including backend-neutral base-scene non-mutation, update metadata, bounded/progress-aware downloads, cancellation cleanup, project migrations, numerical invariance, import, presets, and release-source contracts.
- The Windows/PySide6 offscreen GUI suite passed 112 tests for the Issue #104 candidate, including the production Matplotlib path consuming the extracted base scene.
...
- PyQtGraph production adapter behind an opt-in Windows 11 setting with Matplotlib fallback. Base trace/Y-axis/legend/B% scene composition is backend-neutral and consumed by the current GUI; overlay/interaction commands still need extraction. Changing defaults still requires compatibility evidence.
