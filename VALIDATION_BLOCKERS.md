...
- The non-GUI suite passed 124 tests for the Issue #134 candidate from the repository root, including B% range preservation during shared axis pan, the root-layout and workflow contracts, normalized backend-neutral overview full/detail-window state, bounded home/back/forward history, axis-pan state/calculations, immutable pointer coordinates and hit targets, optional-renderer isolation/pinning, backend-neutral scene contracts, update metadata, project migrations, numerical invariance, import, presets, and release-source contracts.
- The Windows/PySide6 offscreen GUI suite passed 122 tests for the Issue #134 candidate. New MainWindow coverage exercises opt-in switching, real Qt wheel/pan delivery, configured Y-only zoom on both axes, shared toolbar history/reset, repeated scene replacement, independent Y2 overview, snapshots and PNG figure export, raw-array invariance, empty-project reset, tool/split fallback, and initialization/render/event/snapshot failure cleanup. Previous optional-renderer and default-Matplotlib tests also pass. This is not physical mouse/device validation.
- Issue #134 passed syntax compilation and `scripts/ci_validate.py --offline-assets required`, including the locally supplied Windows 7 manifest/wheelhouse. Offscreen MainWindow PNGs were visually inspected with local Meiryo explicitly registered; optional-renderer axis/tick spacing was corrected. Native desktop fonts/DPI, lower-control sizing and physical printing remain open as recorded above. No installer or physical OS/device validation was performed.
...
- Issue #134 connects a session-only experimental PyQtGraph MainWindow view with shared navigation, current-view snapshots, and automatic Matplotlib fallback for unsupported editing/split-view/plot configuration and renderer failures. Next source work is native editing and split-view parity, exact legend/font/tick presentation, removal of hidden Matplotlib screen construction, and integrated performance measurement. Default changes still require physical compatibility evidence.
