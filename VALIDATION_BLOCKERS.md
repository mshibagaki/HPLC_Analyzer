...
- The non-GUI suite passed 124 tests for the Issue #128 candidate from the repository root, including the root-layout and workflow contracts, normalized backend-neutral overview full/detail-window state, bounded home/back/forward history, axis-pan state/calculations, immutable pointer coordinates and hit targets, optional-renderer isolation/pinning, backend-neutral scene contracts, update metadata, project migrations, numerical invariance, import, presets, and release-source contracts.
- The Windows/PySide6 offscreen GUI suite passed 118 tests for the Issue #128 candidate from the repository root, including overview/detail patch updates and click navigation, toolbar history, raw and normalized pan paths, marker/annotation targets, normalized pointer paths, and the optional PyQtGraph consumer's Qt-to-shared pointer mapping, axis-region routing, independent X/Y1/Y2/B% ranges, overview detail window, and non-empty 800×500 Qt snapshot.
...
- PyQtGraph MainWindow adapter behind an opt-in Windows 11 setting with Matplotlib fallback. The optional consumer renders the complete static production scene, applies independent X/Y1/Y2/B% plus overview state, and maps Qt scene positions/axis regions into the shared pointer contract. Widget event subscription and navigation-action routing remain unconnected; MainWindow backend selection and fallback also remain unconnected. Changing defaults still requires compatibility evidence.
