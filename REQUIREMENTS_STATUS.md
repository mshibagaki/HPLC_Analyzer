...
Updated: 2026-08-29 (Issue #158 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | The session-only Qt 6 preview supports shared navigation, single/overview/split views, independent Y axes, scene refresh and snapshots. Native vertical markers, fraction ranges, integration editing/splitting, trace movement, text-label editing and rectangle zoom share existing display history, Undo/Redo and project persistence as applicable. Rectangle zoom supports X, Y, X+Y and automatic modes; split-view Y zoom changes only the dragged panel. Issue #158 keeps the preview active for both toolbar configuration entries and reuses the persisted axes/label-style dialog, including cancel and Undo/Redo behavior; legacy toolbar dialogs remain unchanged outside the preview. Scientific edits use existing callbacks and invalid or changed-target edits roll back atomically. Split-view B% remains both-panels-or-neither. Renderer failures still return to Matplotlib. Win7 and defaults stay unchanged. Hidden Matplotlib objects remain for compatibility/export; this is not a completed high-performance replacement | Complete exact presentation parity (legend placement/fonts/ticks), then eliminate hidden Matplotlib screen construction and benchmark the integrated application. Keep the experiment opt-in and do not change the default until physical compatibility evidence exists |
