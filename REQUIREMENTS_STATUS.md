...
Updated: 2026-08-28 (Issue #130 delivery)
...
| High | Replace the interactive Matplotlib screen renderer | Production-used scene covers all static content. The optional PyQtGraph consumer applies independent view ranges and overview state, renders overlays/snapshots, and dispatches Qt press/double-click/move/release/wheel events as `ScreenPointerEvent` through connect/disconnect IDs. Its viewport filter observes events without suppressing native Qt behavior. Hit targets, pan, bounded history, and overview have backend-neutral contracts. Application navigation-action routing and MainWindow backend selection/fallback remain unconnected | Connect application navigation actions with explicit control of native pan/zoom to avoid duplicate handling, then expose the renderer behind an opt-in Windows 11 setting with automatic Matplotlib fallback; do not change the default until physical compatibility evidence exists |
