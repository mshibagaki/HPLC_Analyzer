...
| High | Replace the interactive Matplotlib screen renderer | The session-only Qt 6 preview supports shared navigation, all display layouts, existing scientific editing workflows and native presentation parity. Issues #162/#164/#166 remove hidden screen artists, add peak-preserving reduction and reuse compatible trace/static items with safe targeted/full rebuilds. Issue #168 adds production discovery/parser benchmarking for arbitrary GCD/TXT files and directories. Issue #176 closes the known coordinate, stuck-pan, pointer-geometry, Y-full and committed trace-translation input regressions with raw-data-invariant coverage. Decorated tracked ASCII evidence (3 traces/162,004 points) measured 0.078 s native versus 0.138 s legacy (1.76x); local untracked paired GCD/TXT evidence (12 traces/203,500 points) measured 0.118 s versus 0.199 s (1.68x), with equal raw SHA-256. Non-chromatogram sidecar TXT files are explicitly reported and skipped. Win7 and defaults stay unchanged | Collect native desktop DPI/font/pointer/clipboard/print evidence on the Windows 11 target and confirm representative longer interactive sessions before deciding whether the Windows 11 default can change |
